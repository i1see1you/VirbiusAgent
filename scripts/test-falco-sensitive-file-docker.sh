...
# ── Config ──
FALCO_IMAGE="falcosecurity/falco:0.41.0"
...
# ── Step 2: Prepare Falco config ──
info "Step 2: Prepare Falco configuration"

# Create a runtime directory for rules only (don't mount custom falco.yaml
# — it interferes with falcoctl driver auto-detection)
...
# Use default Falco config; enable HTTP output via environment variable
if $ENGINE_REACHABLE; then
  info "HTTP output → $FALCO_HTTP_OUTPUT_URL (via FALCO_HTTP_OUTPUT_URL env)"
  export FALCO_HTTP_OUTPUT_ENABLED=true
  ok "HTTP output enabled"
else
  info "HTTP output disabled (Engine not running)"
  export FALCO_HTTP_OUTPUT_ENABLED=false
fi
echo ""
...
DOCKER_RUN_ARGS=(
  ...
  --ulimit memlock=-1:-1
  -e FALCO_HTTP_OUTPUT_URL="$FALCO_HTTP_OUTPUT_URL"
  -e FALCO_HTTP_OUTPUT_ENABLED="${FALCO_HTTP_OUTPUT_ENABLED:-false}"
  -v /dev:/dev
  -v /proc:/host/proc:ro
  -v /sys:/host/sys:ro
  -v /usr:/host/usr:ro
...
# Conditionally mount BPF-related paths for modern_ebpf (required on some kernels)
if [[ -d /sys/kernel/btf ]]; then
  DOCKER_RUN_ARGS+=( -v /sys/kernel/btf:/sys/kernel/btf:ro )
fi
if [[ -d /sys/kernel/tracing ]]; then
  DOCKER_RUN_ARGS+=( -v /sys/kernel/tracing:/sys/kernel/tracing:rw )
fi
if [[ -d /sys/fs/bpf ]]; then
  DOCKER_RUN_ARGS+=( -v /sys/fs/bpf:/sys/fs/bpf:rw )
fi

# Falco 0.41.0+ does NOT have --userspace or --modern-bpf as falco CLI flags.
