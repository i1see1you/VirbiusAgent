...
## 🛡️ VirbiusAgent 防护引擎（真实接入，非模拟）

OWASP lab 顶部的全局「VirbiusAgent 防护」开关开启后，用户输入会先经过**真实的 virbius-core 端层扫描**（matcher + enforce 均在 Rust 内完成，拦截决策不经过模拟），命中 `deny + enforce_mode=full` 的规则即直接拦截。

### 1. 构建并安装真实引擎扩展（一次性）
```bash
cd virbius-mcp-python   # 仓库根目录下
maturin build --release
pip install --force-reinstall ../target/wheels/virbius_mcp_python-*.whl
```
...
其余字段（`tenant_id` / `app_id` / `edge_api_key` / `device_id` / `cache_dir`）对齐 virbius-core 的 `EdgeInitConfig`。切换后重启服务生效。
