<template>
  <div class="v-card">
    <div class="tenants-title-row">
      <h2 class="v-card-title">{{ t('tenants.title') }}</h2>
      <el-tooltip placement="top" :show-after="200" :z-index="4200" popper-class="tenants-help-popper">
        <template #content>
          <p>{{ t('tenants.desc-short') }}</p>
          <p v-html="t('tenants.desc')"></p>
        </template>
        <button type="button" class="tenants-help" :aria-label="t('common.learn-more')">?</button>
      </el-tooltip>
    </div>
...
<style scoped>
.tenants-title-row { display: flex; align-items: center; gap: 8px; margin-bottom: 12px; }
.tenants-title-row .v-card-title { margin: 0; }
.tenants-help {
  width: 22px; height: 22px; flex-shrink: 0; border-radius: 50%;
  border: 1px solid var(--v-border); background: #f8fafc; color: var(--v-muted);
  cursor: pointer; font-size: 12px; line-height: 1; padding: 0;
}
.tenants-help:hover, .tenants-help:focus-visible { color: var(--v-primary); border-color: var(--v-primary); outline: none; }
</style>
<style>
.tenants-help-popper { max-width: 420px; line-height: 1.55; }
.tenants-help-popper p { margin: 0; }
.tenants-help-popper p + p { margin-top: 8px; }
.tenants-help-popper code { font-size: 12px; }
</style>
