...
/**
 * Checkpoint repository for trace stream ingestion.
 * Mirrors {@link AuditIngestCheckpointRepository} but uses {@code tb_trace_ingest_checkpoint}.
 */
@Repository
public class TraceIngestCheckpointRepository {

    private final JdbcTemplate jdbc;
    private final SqlDialectConfig dialect;

    public TraceIngestCheckpointRepository(JdbcTemplate jdbc, SqlDialectConfig dialect) {
        this.jdbc = jdbc;
    ...
    public Optional<StreamEntryID> load(String streamKey) {
        return jdbc.query(
                        """
                        SELECT last_entry_id FROM tb_trace_ingest_checkpoint
                        WHERE stream_key = ?
                        """,
    ...
    public void save(String streamKey, StreamEntryID entryId) {
        ...
        if (dialect.isMysql()) {
            upsertSql = """
                    INSERT INTO tb_trace_ingest_checkpoint (stream_key, last_entry_id, updated_at)
